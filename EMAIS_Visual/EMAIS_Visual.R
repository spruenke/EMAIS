...
################################
      ...
      des = as.data.frame(matrix(nrow = length(P.r), ncol = 10))
...
###########################
      ...
      normal.test = lapply(P.r, shapiro.test) #Test for normality using shapiro wilk test
      
      normal.res = round(as.numeric(lapply(normal.test, "[[", 2)),4) #extract p-value
      skewness.res = abs(round(as.numeric(lapply(P.r, skewness)),4)) #Skewness of Portfolios
      
      T.1 = lapply(P.r[-1], t.test, mu = mean(P.r[[1]])) #two sided t-test, external mean is the mean of the naive portfolio
...
#############################
      ...
      psi.res = as.data.frame(matrix(nrow = length(P.r), ncol = 4))
      for(i in 1:length(P.r)){
        psi.res[i,] = psi(P.r[[i]], W[[i]], gamma = gam)
      }
      
      #create gross returns
      P.gross = lapply(P.r, FUN = function(x){
        1+x
      })
      ...
      save(ceq.1, sr.1, asr.1, to.1, tr.1, file = "metrics_cost_div2.RData")
      ...
      #xtable(psi.res.2)
      save(psi.res, file = "./res/psi-results.RData")
      options(scipen = 0)
      
      sink(file = "./res/psi-res.txt")
      print(xtable(xres.2), sanitize.text.function=function(x){x})
      sink()
...
###############
      ...
      g.min = min(sapply(perf, min), na.rm = T) #global minimum
      g.max = max(sapply(perf, max), na.rm = T) #global maximum
      
      plot(perf[[1]], xlab = "Time", ylab = "Return", main = "Cumulative Performance", ylim = c(g.min, g.max), type = "l", lwd = 2.3)
      for(i in 2:length(perf)){
        lines(perf[[i]], col = rainbow(length(perf))[i], lwd = 2.3)
      }
      
      pdf(file = "./res/performance.pdf", width = 9, height = 6)
      plot(perf[[1]], xlab = "Time", ylab = "Wealth", main = "Cumulative Performance", ylim = c(g.min, g.max), type = "l", lwd = 2.3)
      for(i in 2:length(perf)){
        lines(perf[[i]], col = rainbow(length(perf))[i], lwd = 2.3)
      }
      dev.off()
      ...
      names(P.r)[which(sapply(perf, tail, 1)==max(sapply(perf, tail, 1), na.rm = T))]
      
      #plot a legend for the plot above
      pdf(file = "./res/legend.pdf", width = 9, height = 6)
      plot(NULL ,xaxt='n',yaxt='n',bty='n',ylab='',xlab='', xlim=0:1, ylim=0:1)
      legend("topleft", legend =names(P.r), pch=16, bty='n', cex = 0.9,
             col = rainbow(length(names(P.r))))
      mtext("Strategies", at=0.115, cex=2)
      dev.off()
      
      for(i in 1:length(perf)){
        pdf(file = paste0("./res/performance/", names(P.r)[i], ".pdf"), width = 9, height = 6)
        plot(perf[[i]], xlab = "Time", ylab = "Wealth", main = "Cumulative Performance", type = "l", lwd = 2.3, xaxt = "n")
        axis(1, at = c(0,200,400,600,800, 892), labels = dates.tck[c(M+1, M+200, M+400, M+600, M+800, M+892)])
        dev.off()
      }
      ...
      #Plot
      pdf(file = "./res/performance_1.pdf", width = 9, height = 6)
      ...
      #Boxplots
      #Prepare
      mu = as.numeric(lapply(P.r, mean, na.rm = T))
      sig = as.numeric(lapply(P.r, sd, na.rm = T))
      med.1 = as.numeric(lapply(P.r, median, na.rm = T))
      iqr.1 = as.numeric(lapply(P.r, IQR, na.rm = T))
      ...
      #plots
      pdf(file = "./res/boxplot_mean.pdf", width = 9, height = 6)
      ...
      for(i in 1:length(perf)){
        if(all(!is.na(P.r[[i]]))){
          pdf(file = paste0("./res/density/", names(P.r)[i], ".pdf"), width = 9, height = 6)
          plot(density(P.r[[i]]*100), main = "Kernel Density Estimate", type = "l", lwd = 3, xlab = "Return in %")
          abline(v = mean(P.r[[i]]), lwd = 3, col = "red")
          abline(v = median(P.r[[i]]), lwd = 3, col = "blue")
          dev.off()} else {
            next
          }
      ...
      x.min = min(sig.1)
      x.max = max(sig.1)
      pdf(file = "./res/scatter_est.pdf", width = 9, height = 6)
      ...
      pdf(file = "./res/scatter_strat.pdf", width = 9, height = 6)
      ...
      pdf(file = "./res/scatter_total.pdf", width = 9, height = 6)
      ...
      points(sig[1], mu[1], cex = 3, pch = 19)
      dev.off()